######Classification models based on paDEL descriptors using RF-VIM2 (mdg)-ranked top 30 (corr. coeff. <0.75)
setwd("~/Desktop/ML_mTOR/mTORFSRFVIM2_3")
library(data.table)
library(dplyr)
...
summary(mtor.impvariable2)
dim(mtor.impvariable2[mtor.impvariable2$MeanDecreaseGini>1.56, ])
...
mtor.vim2 <- (mtor.impvariable2[mtor.impvariable2$MeanDecreaseGini>1.56, ])$variable
...
model_rf30pp.RDS = readRDS("model_rf30pp.RDS")
...
#pred_rf30pptest <- predict(model_rf30pp, newdata = testdata[,!(colnames(testdata)) %in% c("y")], classProbs = TRUE)
...
model_rp30.RDS = readRDS("model_rp30.RDS")
...
#pr.rftr30 = predict(model_rf30pp, newdata = train, type="prob")[,2]
...
#bg_rf30 <-pred_rf30pptest[mtorfs.test$label == 0]
length(bg_rf30)
